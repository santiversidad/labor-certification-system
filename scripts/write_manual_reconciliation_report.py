"""Assemble the final report only after complete verification evidence exists."""
import json
import re
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
read_json = lambda p: json.loads((ROOT / p).read_text(encoding='utf-8-sig'))
debug = (ROOT / 'tmp/reconciliation-backend-debug.txt').read_text(encoding='utf-8-sig')
passed = re.findall(r'^Test Passed \((.+)\)$', debug, re.M)
assert len(passed) == 117 and len(set(passed)) == 117
assert 'PHPUnit Finished (Shell Exit Code: 0)' in debug
assert not re.search(r'^Test (Failed|Errored|Skipped|Marked Incomplete)', debug, re.M)
front = (ROOT / 'tmp/reconciliation-frontend.txt').read_text(encoding='utf-8-sig')
build = (ROOT / 'tmp/reconciliation-build.txt').read_text(encoding='utf-8-sig')
assert '15 passed' in front and 'built in' in build
verification = read_json('MANUAL_RECONCILIATION_VERIFICATION.json')
dry = read_json('MANUAL_RECONCILIATION_DRY_RUN.json')
metadata = read_json('backend/laravel-app/database/data/manual_funciones_decreto_015_2023_reconciliado.json')['metadata']
docker_lines = (ROOT / 'tmp/reconciliation-docker-after.json').read_text(encoding='utf-8-sig').splitlines()
containers = [json.loads(s) for s in docker_lines if s.strip()]
assert len(containers) == 4 and all(c['Health'] == 'healthy' for c in containers)
assert verification['estado_persistente_igual_al_preflight'] and verification['fuentes_originales_intactas']
assert dry['source_ids']['actualizadas'] == ['MF-0167'] and not dry['functions_diff']
sha_xlsx = next(s['sha256'] for s in metadata['fuentes'] if s['formato'] == 'xlsx')
text = f'''# Resultado de conciliación del Manual de Funciones
...
| 2. SHA-256 | `{sha_xlsx}`. Hashes JSON y PDF en la inspección y metadata del derivado. |
...
'''
(ROOT / 'MANUAL_RECONCILIATION_REPORT.md').write_text(text, encoding='utf-8')

p = ROOT / 'MANUAL_IMPORT_REPORT.md'
old = p.read_text(encoding='utf-8')
old = old.replace('El Excel no se localizó, falta contrastar ambas fuentes, el JSON omite campos presentes en el PDF y no hay fecha de expedición/vigencia confirmada.',
                  'Excel/JSON ya están conciliados (344 fichas iguales). Ambos omiten campos presentes en PDF, MF-0167 requiere revisión y no hay fecha de expedición/vigencia confirmada. Resultado actual: **NO PUBLICABLE**; véase MANUAL_RECONCILIATION_REPORT.md.')
old = old.replace('**No localizado**. Búsqueda recursiva del repositorio, incluidos ignorados, carpeta padre y Excel de Descargas. Se solicitó ruta/nombre al usuario.',
                  'Localizado en `C:/Users/mondr/Downloads/manual_funciones_decreto_015_2023.xlsx`; copia idéntica en `backend/laravel-app/database/data/`. SHA-256 y esquema en MANUAL_SOURCE_INSPECTION.md.')
old = old.replace('Hojas, encabezados, filas, tipos y tres fichas **no verificables sin archivo**. No se inventaron.',
                  'LEEME 9×2; Perfiles_Cargo 345×14; Funciones 3.116×13; Conocimientos 2.327×8. Inspección completa y tres ejemplos verificados disponibles.')
old = old.replace('**Comparación pendiente, no se afirma equivalencia**. Detallada en `MANUAL_IMPORT_COMPARISON.md`.',
                  '**344 fichas COINCIDE; 0 diferencias de campos/funciones, 0 exclusivas, 0 ambiguas**. Contraste PDF puntual en `MANUAL_IMPORT_COMPARISON.md`.')
old = old.replace('**tres registros íntegros**', '**tres ejemplos con campos completos y arrays abreviados**')
old = old.replace('ausencia de secciones del Manual/comparación Excel.', 'ausencia de secciones del Manual. La comparación Excel posterior ya confirmó igualdad de ambas fuentes estructuradas.')
old = old.replace('1. Localizar el Excel y comparar registros/campos/funciones, sin corregir automáticamente ninguna fuente.',
...
old = old.replace('Incidencia operativa al cierre:', '**Incidencia histórica, resuelta en la conciliación posterior (los cuatro servicios están healthy):**')
marker = '## Actualización: conciliación Excel / JSON / PDF'
...
print('Informe final de conciliación creado; informe de importación actualizado. 117 backend / 15 frontend / build OK.')
