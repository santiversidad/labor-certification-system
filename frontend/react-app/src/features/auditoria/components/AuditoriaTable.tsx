...
export function AuditoriaTable({ logs, loading = false }: { logs: AuditLog[]; loading?: boolean }) {
  ...
  const metadata = selected ? Object.entries(selected.metadata ?? {}).filter(([, value]) => ['string', 'number', 'boolean'].includes(typeof value)) : [];
  return <>
