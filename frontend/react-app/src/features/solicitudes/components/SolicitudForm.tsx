      ...
      <div className="p-5 sm:p-8">
        {screen === 'select' ? (
          <div className="space-y-7">
            <div><p className="eyebrow">Paso 1 de 4</p><h2 className="mt-2 text-xl font-bold text-text">Seleccione la certificación</h2><p className="mt-1 text-sm text-muted">Elija entre la constancia básica de vinculación o el documento que incorpora las funciones del empleo.</p></div>
            <fieldset>
              <legend className="text-sm font-semibold text-text">Tipo de certificado</legend>
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                {([
                  { value: 'sencillo', title: 'Sencillo', description: 'Constancia de vinculación y empleo, sin relación de funciones.' },
                  { value: 'funciones', title: 'Con funciones', description: 'Incluye las funciones del empleo según el Manual aplicable.' },
                ] as const).map((option) => (
                  <button aria-pressed={tipoCertificado === option.value} className={`flex min-h-24 items-center justify-between rounded-lg border p-4 text-left transition ${tipoCertificado === option.value ? 'border-primary bg-primary/5 shadow-sm' : 'border-border hover:border-secondary/50'}`} key={option.value} onClick={() => setValue('tipo_certificado', option.value, { shouldValidate: true })} type="button">
                    <span><strong className="block text-text">{option.title}</strong><small className="mt-1 block leading-5 text-muted">{option.description}</small></span>
                    {tipoCertificado === option.value ? <Check className="shrink-0 text-primary" size={20} /> : null}
                  </button>
                ))}
              </div>
            </fieldset>
          ...
          <div className="space-y-7">
            ...
            <Alert title="Antes de confirmar" tone="info">El sistema validará el cupo mensual y la vinculación laboral. Para el certificado con funciones también validará la ficha aplicable del Manual. Si la configuración exige pago, creará una orden pendiente.</Alert>
