  ...
  return (
    <div className="app-page max-w-6xl">
      ...
      <div className="flex items-start gap-3 border-y border-border py-4 text-sm leading-6 text-muted">
        ...
        <p>Dispone de un certificado sencillo y uno con funciones por mes calendario. La información proviene de las fuentes institucionales vigentes.</p>
      </div>
      {query.isError ? <ErrorState message="No fue posible consultar la disponibilidad de certificaciones." /> : null}
      {query.isLoading ? <div className="grid gap-5 md:grid-cols-2" aria-label="Cargando modalidades"><Skeleton className="h-72" /><Skeleton className="h-72" /></div> : null}
      {query.data ? (
        <div className="grid gap-5 md:grid-cols-2">
          <CertificationOption availability={query.data.data.sencillo} description="Constancia de vinculación y empleo, sin relación de funciones." icon={FileText} note="Generación inmediata cuando no se exige pago." title="Certificado laboral sencillo" to="/app/solicitudes/nueva?modalidad=sencillo" />
          <CertificationOption availability={query.data.data.funciones} description="Incluye la información laboral y las funciones correspondientes al empleo según el Manual de Funciones aplicable." icon={ListChecks} note="La ficha se resuelve automáticamente y se valida antes de generar el documento." title="Certificado laboral con funciones" to="/app/solicitudes/nueva?modalidad=funciones" />
        </div>
      ) : null}
      <p className="text-xs text-muted">Este portal no expone expedientes ni actuaciones administrativas internas.</p>
    </div>
  );
}
