...
export function ValidationResultCard({ result }: { result: ValidacionCertificado }) {
  ...
  const integridadComprometida = result.resultado === 'integridad_comprometida';
  const noEncontrado = result.resultado === 'no_encontrado';
  const mensaje = noEncontrado
      ...
      <dl className="grid gap-3 text-sm sm:grid-cols-2">
        <div>
          ...
          <dd>
            <Badge tone={result.valido ? 'green' : 'red'}>
              {anulado ? 'CERTIFICADO ANULADO' : result.valido ? 'Válido' : 'No válido'}
            </Badge>
          </dd>
        </div>
        <div>
        ...
        {result.funcionario ? (
          <div>
            ...
            <dd className="font-medium text-text">{result.funcionario.nombre}</dd>
          </div>
        ) : null}
        {result.cargo ? (
          <div>
        ...
        {result.fecha_generacion ? (
          <div>
            <dt className="text-muted">Fecha de generación</dt>
            <dd className="font-medium text-text">{result.fecha_generacion}</dd>
          </div>
        ) : null}
      </dl>
